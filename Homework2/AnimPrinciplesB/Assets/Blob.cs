...
public class Blob : MonoBehaviour 
{
    ...
    public bool DestoryDoors = false;
    ...
    void Explode()
    {
        ...
        if (StuckOn != null && StuckOn.name == "Door")
        {
            float ang = 0;
            Vector3 boomVec = Vector3.zero;
            
            ExplosionDirecton.ToAngleAxis(out ang,out boomVec);
            StuckOn.rigidbody.constraints = RigidbodyConstraints.None;
            StuckOn.rigidbody.AddForceAtPosition(boomVec * 100, ExplosionLocation);
            if (DestoryDoors)
                Destroy(StuckOn, ExplosionLife*2);
        }
    }
